...
public class NewsListAdapter extends BaseMultiItemQuickAdapter<News, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder baseViewHolder, News news) {
        ...
        baseViewHolder.setText(R.id.tv_title, news.title)
                ...
                .setText(R.id.tv_time, TimeUtils.getFriendlyTimeSpanByNow(news.behot_time * 1000))
                .setText(R.id.tv_author_none, news.source)
                .setText(R.id.tv_comment_num_none, news.comment_count + "评论")
                .setText(R.id.tv_time_none, TimeUtils.getFriendlyTimeSpanByNow(news.behot_time * 1000));
        ...
        switch (baseViewHolder.getItemViewType()) {
            ...
            case News.TYPE_CENTER_PIC_NEWS:
                ...
                if (news.has_video) {
                    ...
                    baseViewHolder.setText(R.id.tv_bottom_right, TimeUtils.millis2String(news.video_duration, new SimpleDateFormat("mm:ss", Locale.getDefault())));//设置时长
                    //中间图片使用视频大图
                    ImageLoader.getInstance().displayImage(mContext, news.video_detail_info.detail_video_large_image.url, (ImageView) baseViewHolder.getView(R.id.iv_img));
                } else {
                    ...
                    //中间图片使用image_list第一张
                    ImageLoader.getInstance().displayImage(mContext, news.image_list.get(0).url.replace("list/300x196", "large"), (ImageView) baseViewHolder.getView(R.id.iv_img));
                }
                break;
            case News.TYPE_RIGHT_PIC_NEWS:
                ...
                if (news.has_video) {
                    ...
                    baseViewHolder.setText(R.id.tv_duration, TimeUtils.millis2String(news.video_duration, new SimpleDateFormat("mm:ss", Locale.getDefault())));//设置时长
                } else {
                    baseViewHolder.setVisible(R.id.ll_duration, false);//隐藏时长
                }
                ImageLoader.getInstance().displayImage(mContext,news.middle_image.url, (ImageView) baseViewHolder.getView(R.id.iv_img));
                break;
            case News.TYPE_THREE_PIC_NEWS:
                //三张图片的新闻
                ImageLoader.getInstance().displayImage(mContext, news.image_list.get(0).url, (ImageView) baseViewHolder.getView(R.id.iv_img1));
                ImageLoader.getInstance().displayImage(mContext, news.image_list.get(1).url, (ImageView) baseViewHolder.getView(R.id.iv_img2));
                ImageLoader.getInstance().displayImage(mContext, news.image_list.get(2).url, (ImageView) baseViewHolder.getView(R.id.iv_img3));
                break;
        }
    }
}
